...
void OptionsState::initialize () {
    ...
    OKButton = new Button("assets/button-ok.png", &Game::goBack, (config::SCREEN_WIDTH - 80) / 2, 280);
}
...
void OptionsState::draw () {
    ...
    string resolution_string = to_string(int(config::SCREEN_WIDTH*config::resolution_scaling)) + " x " +
        to_string(int(config::SCREEN_HEIGHT*config::resolution_scaling));

    resolution_text->loadFromText (resolution_string, Game::getInstance()->mRenderer->mediumFont, config::default_text_color);
    resolution_text->render(400+(200-resolution_text->getWidth())/2, 100);
    ...
    if (index <= 1) {
        ...
        SDL_Rect highlight_box = {0, 100+(index*80), config::SCREEN_WIDTH, height+5};
...
void OptionsState::changeResolution (SettingChange s) {
    if (s == SettingChange::left && resolution_scaling_index > 0) {
        ...
        SDL_SetWindowSize(Game::getInstance()->mWindow, config::SCREEN_WIDTH * config::resolution_scaling,
            config::SCREEN_HEIGHT * config::resolution_scaling);
