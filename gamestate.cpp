...
void GameState::initialize () {
    ...
    score_text->loadFromText("Score:", Game::getInstance()->mRenderer->mediumFont, config::default_text_color);
    ...
    for (int i = 0; i < 7; i++) {
        tetrominoSpriteClips[i].x = config::block_size*i;
        tetrominoSpriteClips[i].y = 0;
        tetrominoSpriteClips[i].w = config::block_size;
        tetrominoSpriteClips[i].h = config::block_size;
    }
    ...
    switch (currentPhase) {
        case GAME_STARTED: {
            ...
            if (ms_passed < 3000) {
                ...
                if (countdown_time >= 0) {
                    countdown_texture->loadFromText(to_string(countdown_time), Game::getInstance()->mRenderer->mediumFont, config::default_text_color);
                    Game::getInstance()->mRenderer->renderTexture(countdown_texture, config::SCREEN_WIDTH/2, config::SCREEN_HEIGHT/2);
                }
        ...
        case GAME_PLAYING: {
            score_string = to_string(config::score);
            Score->loadFromText(score_string, Game::getInstance()->mRenderer->mediumFont, config::default_text_color);
            ...
            if (mInputManager->isGameExiting()) {
                ...
                if (time_snap2 - time_snap1 >= double(config::wait_time) / (config::score / 5000 + 1)) {
                    movePieceDown();
                    time_snap1 = SDL_GetTicks();
                }
                ...
                Game::getInstance()->mRenderer->clearScreen();
                draw();
                Game::getInstance()->mRenderer->renderTexture(Score, config::SCREEN_WIDTH/2 - 200, config::SCREEN_HEIGHT/2 - 30);
                Game::getInstance()->mRenderer->renderTexture(score_text, config::SCREEN_WIDTH/2 - 200, config::SCREEN_HEIGHT/2 - 50);
                Game::getInstance()->mRenderer->updateScreen();
                Score->free();
            }
            else {
        ...
        case GAME_FINISHED: {
            if (!mInputManager->isGameExiting()) {
                ...
                draw();
                Game::getInstance()->mRenderer->renderTexture(gameover_text, config::SCREEN_WIDTH/2 + 200, config::SCREEN_HEIGHT/2);
                Game::getInstance()->mRenderer->updateScreen();
            }
            else {
...
void GameState::drawNextPiece (Piece p) {
    for (int row = 0; row < config::matrix_blocks; row++) {
        for (int col = 0; col < config::matrix_blocks; col++) {
            if (p.getBlockType(row, col) != 0) {
                tetrominoSprites->render(config::next_box_x + col*config::block_size, config::next_box_y +
                    row*config::block_size, &tetrominoSpriteClips[p.piece_type]);
            }
        }
    }
}
...
void GameState::drawHoldPiece (Piece p) {
    for (int row = 0; row < config::matrix_blocks; row++) {
        for (int col = 0; col < config::matrix_blocks; col++) {
            if (p.getBlockType(row, col) != 0) {
                tetrominoSprites->render(config::hold_box_x + col*config::block_size, config::hold_box_y +
                    row*config::block_size, &tetrominoSpriteClips[p.piece_type]);
            }
        }
    }
...
int GameState::getRandom (int minn, int maxx) {
